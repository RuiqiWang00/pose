...
class Compose(object):
	...
	def __call__(self, image, joints, bbox):
		ori_img=image
		ori_joints=joints
		for t in self.transforms:
			image,ori_img, joints,ori_joints, bbox = t(image,ori_img ,joints, ori_joints, bbox)
		return image,ori_img, joints,ori_joints, bbox
...
class ToTensor(object):
	def __call__(self, image,ori_img, joints,ori_joints, bbox):
		return F.to_tensor(image), ori_img,joints,ori_joints, bbox
...
class RandomHorizontalFlip(object):
	def __init__(self, flip_index,input_size, output_size, prob=0.5):
	...
	def __call__(self, image,ori_img, joints,ori_joints, bbox):
		if random.random() < self.prob:
			...
			ori_joints = ori_joints[:, self.flip_index]
			ori_joints[:, :, 0] = self.input_size - ori_joints[:, :, 0] - 1
...
class RandomAffineTransform(object):
	...
	def __call__(self, image,ori_img, joints,ori_joints, bbox):
		...
		ori_joints[:,:,0:2]=self._affine_joints(ori_joints[:,:,0:2],mat_input)
		image = cv2.warpAffine(image, mat_input, (self.input_size, self.input_size))

		ori_img=image.copy()

		#print('ori_kp',ori_joints,'joints',joints,sep='\n')
